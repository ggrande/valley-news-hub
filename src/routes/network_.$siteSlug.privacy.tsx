...
export const Route = createFileRoute("/network_/$siteSlug/privacy")({
  loader: async ({ params }) => {
    const page = await getTenantLegalPage({ data: { slug: params.siteSlug, kind: "privacy" } });
    if (!page) throw notFound();
    return { page };
  },
  head: ({ params, loaderData }) => {
    const url = `https://network.wkna49.com/network/${params.siteSlug}/privacy`;
    const name = loaderData?.page?.siteName ?? params.siteSlug;
    return {
      meta: [
        { title: `Privacy Policy — ${name}` },
        { name: "description", content: `How ${name} handles reader information, cookies, and newsletter data.` },
...
function TenantPrivacy() {
  ...
  return (
    <Layout>
      ...
      <TenantHeader tenant={tenant} title={page.title} description={`Privacy practices at ${page.siteName}.`} />
